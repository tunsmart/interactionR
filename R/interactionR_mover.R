...
#' @param  coef A vector of two numbers indicating the position of the two
#'   interaction terms in the model summary. The default is 2 and 3, which
#'   assumes that the two exposures are listed first before any additional
#'   covariates
#' @param  CI.level A number indicating the confidence level for the CI
#'   estimation. The default is 0.95 CI
#' @param  type A letter 'e' for effect modification or 'i' for interaction. The
#'   default is 'e'.
#' @return A dataframe containing all the neccessary estimates required to full
#'   report effect modification or interaction with the CI estimates for
#'   interaction measures computed using the variance recovery method
#' @examples
...
interactionR_mover = function(model, exposure_names = c(), ci.level = 0.95, em = T,
    ...
    d = data.frame(Measures = c("OR00", "OR01", "OR10", "OR11", paste("OR(",
        ...
        "RERI"), Estimates = c(OR00, OR01, OR10, OR11, OR01, OR_X1, OR_M,
        RERI), CI.ll = c(NA, l2, l1, l3, l2, CI.ll_OR_X1, CI.ll_OR_M, L),
    ...
    if (!em) {
        d = data.frame(Measures = c("OR00", "OR01", "OR10", "OR11", paste("OR(",
            ...
            APL, SIL), CI.ul = c(NA, u2, u1, u3, u2, CI.ul_OR_X1, u1, CI.ul_OR_A1,
