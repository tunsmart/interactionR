...
#' @export
ei.mover = function(model, coef = c(2, 3), ci.level = 0.95, type = "e", recode = F) {
    ...
    if (preventive(OR10 = exp(b1), OR01 = exp(b2))) {
        ...
        if (recode) {
            ...
            # inform the user
            warning("Recoding exposures; new reference category for ", beta1, " is ",
                    E1.ref, " and for ", beta2, " is ", E2.ref)
    ...
    # their CI with the 'MOVER' method (Zou (2018)
    ...
    vSItheta2 = ((exp(2 * b1) * v1) + (exp(2 * b2) * v2) + (2 * exp(b1 + b2) * cov12))/(exp(b1) +
        exp(b2) - 2)^2
    ...
    if (type == "i") {
        ...
        out = data.frame(Measures = c("OR00", "OR01", "OR10", "OR11", paste("OR(",
            ...
            Estimates = c(OR00, OR01, OR10, OR11, OR01, OR_X1, OR_M, RERI), CI.ll = c(NA,
                l2, l1, l3, l2, CI.ll_OR_X1, CI.ll_OR_M, L), CI.ul = c(NA, u2, u1,
                u3, u2, CI.ul_OR_X1, CI.ul_OR_M, U))
        rownames(out) = NULL
        return(out)
    }
