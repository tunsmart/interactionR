...
ei.delta = function(model, coef = c(2, 3), CI.level = 0.95, type = "e") {
    ...
    # calculates their CI and p-value with the delta method implemented in the msm
    # package
    ...
    if (type == "i") {
        ...
        out = data.frame(Measures = c("OR00", "OR01", "OR10", "OR11", paste("OR(",
            ...
            "RERI", "AP"), Estimates = c(OR00, OR01, OR10, OR11, OR01, OR_X1, OR_M,
            RERI, AP), CI.ll = c(NA, CI.ll_OR01, CI.ll_OR10, CI.ll_OR11, CI.ll_OR01,
            CI.ll_OR_X1, CI.ll_OR_M, CI.ll_RERI, CI.ll_AP), CI.ul = c(NA, CI.ul_OR01,
            CI.ul_OR10, CI.ul_OR11, CI.ul_OR01, CI.ul_OR_X1, CI.ul_OR_M, CI.ul_RERI,
            CI.ul_AP))
        rownames(out) = NULL
        return(out)
    }
